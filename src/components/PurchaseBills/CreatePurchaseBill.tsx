...
const CreatePurchaseBill: React.FC = () => {
  ...
  useEffect(() => {
    if (isEditing && id) {
      ...
      if (bill) {
        ...
        if (bill.discountType) setOverallDiscountType(bill.discountType)
        if (bill.discountValue !== undefined) setOverallDiscountValue(bill.discountValue)
      }
    }
  ...
  const gstBreakdown = calculateGSTBreakdown(taxableAmount, user?.state || "", vendorState)
